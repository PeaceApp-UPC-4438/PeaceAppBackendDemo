...
namespace PeaceApp.API.IAM.Interfaces.ACL.Services;
...
public class IamContextFacade(IUserCommandService userCommandService, IUserQueryService userQueryService) : IIamContextFacade
{
    ...
    {
        var getUsernameByIdQuery = new GetUserByIdQuery(userId);
        var result = await userQueryService.Handle(getUsernameByIdQuery);
